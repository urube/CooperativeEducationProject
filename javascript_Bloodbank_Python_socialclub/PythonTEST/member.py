...
class Member:
    ...
    def add_activity(self, new_name, new_place, new_start_date, new_cost):
        new_activity = Activity(new_name, new_place, new_start_date, new_cost)
        self.all_my_activities.append(new_activity)

    def has_many_activities(self):
        many_activities = False
        if len(self.all_my_activities) == 2:
            many_activities = True
        if len(self.all_my_activities) == 3:
            many_activities = True
