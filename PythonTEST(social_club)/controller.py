...
def setup():
    ...
    # ADD CODE HERE
    club = the_club.find_member('M01')
    club.add_activity('Autumn Festival', 'Arrowtown', date(2018, 1, 1), 0)

    club = the_club.find_member('M05')
    club.add_activity('Joyride Carnival', 'Auckland', date(2019, 5, 2), 300)
    club.add_activity('Easter High Tea', 'Wellington', date(2018, 12, 10), 150)
    club.add_activity('Festival of Colour', 'Hamilton', date(2018, 11, 7), 0)

    club = the_club.find_member('M02')
    club.add_activity('Wine and Food Festival', 'Napier', date(2020, 10, 20), 350)

    club = the_club.find_member('M03')

    club = the_club.find_member('M04')
    club.add_activity('Music in Park', 'Hamilton', date(2018, 12, 8), 0)
